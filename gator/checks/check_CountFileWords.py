"""Check that a file in a directory contains a specified number of words."""
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="CountFileWords",
        description="Check Provided by GatorGrader: CountFileWords",
    ...
    # REQUIRED? Yes
    required_group.add_argument(
        "--count", type=int, metavar="COUNT", help="how many total words should exist in the file"
    )
...
def act(main_parsed_arguments, check_remaining_arguments):
    ...
    # --> count is required to specify the expected number of words in the file
    ...
    # This means that the use of check_CountFileWords would have already failed by this
    ...
    return [invoke.invoke_all_total_word_count_checks(file, directory, count, exact)]
